    ...
    public class CueProfile
    {
        ...
        internal static CueProfile Load(string file)
        {
            ...
            try
            {
                ...
                var profileRoot = XDocument.Load(file).Root;
                return new CueProfile(profileRoot.Element("id").Value, profileRoot.Element("name").Value)
                {
                    _devices = profileRoot.Element("devices").Elements("device").Select(CueProfileDevice.Load).Where(x => x != null).ToDictionary(x => x.Name)
                };
            }
        ...
        public IEnumerable<string> Modes
        {
            get
            {
                var device = _devices.Keys.FirstOrDefault();
        ...
        public ProfileBrush this[string mode]
        {
            get
            {
                var device = CueSDK.KeyboardSDK?.KeyboardDeviceInfo?.Model;
                CueProfileDevice cpd;
