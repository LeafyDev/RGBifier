...
namespace CUE.Net.Brushes
{
    ...
    public abstract class AbstractBrush : AbstractEffectTarget<IBrush>, IBrush
    {
        ...
        public virtual void PerformRender(RectangleF rectangle, IEnumerable<BrushRenderTarget> renderTargets)
        {
            ...
            foreach(var point in renderTargets)
                RenderedTargets[point] =
                    new CorsairColor(GetColorAtPoint(rectangle,
                        point)); // Clone the color, we don't want to have reference issues here and brushes might return the same color multiple times!
        }
        ...
        {
            var renderTargets = RenderedTargets.Keys.ToList();
        ...
        ///   If you overwrite this method please make sure that you never return the same color-object twice to prevent
        ///   reference-issues!
        /// </summary>
        ...
        {
            foreach(var colorCorrection in ColorCorrections)
            ...
            var finalBrightness = color.GetHSVValue() * (Brightness < 0 ? 0 : (Brightness > 1f ? 1f : Brightness));
            var finalAlpha = (byte) (color.A * (Opacity < 0 ? 0 : (Opacity > 1f ? 1f : Opacity)));
