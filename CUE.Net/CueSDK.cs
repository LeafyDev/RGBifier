...
namespace CUE.Net
{
    ...
    public static partial class CueSDK
    {
        ...
        public static List<string> PossibleX64NativePaths { get; } =
            new List<string> {$"{AppContext.BaseDirectory}/x64/CUESDK_2015.dll", $"{AppContext.BaseDirectory}/x64/CUESDK.dll"};
        ...
        /// <summary>
        ///   Checks if the SDK for the provided <see cref="CorsairDeviceType" /> is available or checks if CUE is installed and
        ///   SDK supported enabled if null is provided.
        /// </summary>
        ...
        public static bool IsSDKAvailable(CorsairDeviceType? sdkType = null)
        {
            try
            {
                ...
                if(IsInitialized)
                {
                    ...
                    switch(sdkType)
                    {
                        case CorsairDeviceType.Keyboard: return KeyboardSDK != null;
                        case CorsairDeviceType.Mouse: return MouseSDK != null;
                        case CorsairDeviceType.Headset: return HeadsetSDK != null;
                        case CorsairDeviceType.Mousemat: return MousematSDK != null;
                        default: return true;
                    }
                }
                else
                {
                    ...
                    {
                        var info = new GenericDeviceInfo(Marshal.PtrToStructure<_CorsairDeviceInfo>(_CUESDK.CorsairGetDeviceInfo(i)));
        ...
        ///   Thrown if the SDK is already initialized, the SDK is not compatible to CUE or if CUE
        ...
        public static void Initialize(bool exclusiveAccess = false)
        {
            ...
            for(var i = 0; i < deviceCount; i++)
            {
                ...
                switch(info.Type)
                {
                    ...
                    default: throw new WrapperException("Unknown Device-Type");
                }
        ...
        public static void Reinitialize(bool exclusiveAccess)
        {
            ...
            var deviceCount = _CUESDK.CorsairGetDeviceCount();
            var reloadedDevices = new Dictionary<CorsairDeviceType, GenericDeviceInfo>();
            for(var i = 0; i < deviceCount; i++)
            {
                var info = new GenericDeviceInfo(Marshal.PtrToStructure<_CorsairDeviceInfo>(_CUESDK.CorsairGetDeviceInfo(i)));
                if(!info.CapsMask.HasFlag(CorsairDeviceCaps.Lighting))
            ...
            if(KeyboardSDK != null)
                if(!reloadedDevices.ContainsKey(CorsairDeviceType.Keyboard) || KeyboardSDK.KeyboardDeviceInfo.Model
                   != reloadedDevices[CorsairDeviceType.Keyboard].Model)
            ...
            if(HeadsetSDK != null)
                if(!reloadedDevices.ContainsKey(CorsairDeviceType.Headset) || HeadsetSDK.HeadsetDeviceInfo.Model
                   != reloadedDevices[CorsairDeviceType.Headset].Model)
                    throw new WrapperException("The previously loaded Headset got disconnected.");
            if(MousematSDK != null)
                if(!reloadedDevices.ContainsKey(CorsairDeviceType.Mousemat) || MousematSDK.MousematDeviceInfo.Model
                   != reloadedDevices[CorsairDeviceType.Mousemat].Model)
