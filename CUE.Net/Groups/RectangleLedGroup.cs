...
namespace CUE.Net.Groups
{
    ...
    public class RectangleLedGroup : AbstractLedGroup
    {
        ...
        {
            return _ledCache ?? (_ledCache = Device.Where(x => RectangleHelper.CalculateIntersectPercentage(x.LedRectangle, Rectangle) >= MinOverlayPercentage)
                       .ToList());
        }
        ...
        /// <summary>
        ///   Gets or sets the minimal percentage overlay a LED must have with the <see cref="Rectangle" /> to be taken into the
        ///   ledgroup.
        /// </summary>
        ...
        ///   (optional) The minimal percentage overlay a LED must have with the
        ///   <see cref="Rectangle" /> to be taken into the ledgroup. (default: 0.5f)
        /// </param>
        /// <param name="autoAttach">
        ///   (optional) Specifies whether this group should be automatically attached or not. (default:
        ///   true)
        /// </param>
        public RectangleLedGroup(ICueDevice device, CorsairLedId fromLed, CorsairLedId toLed, float minOverlayPercentage = 0.5f, bool autoAttach = true) : this(
            device, device[fromLed], device[toLed], minOverlayPercentage, autoAttach) { }
        ...
        ///   true)
        /// </param>
        public RectangleLedGroup(ICueDevice device, CorsairLed fromLed, CorsairLed toLed, float minOverlayPercentage = 0.5f, bool autoAttach = true) : this(
            device, RectangleHelper.CreateRectangleFromRectangles(fromLed.LedRectangle, toLed.LedRectangle), minOverlayPercentage, autoAttach) { }
        ...
        ///   true)
        /// </param>
        public RectangleLedGroup(ICueDevice device, PointF fromPoint, PointF toPoint, float minOverlayPercentage = 0.5f, bool autoAttach = true) : this(device,
            RectangleHelper.CreateRectangleFromPoints(fromPoint, toPoint), minOverlayPercentage, autoAttach) { }
        ...
        ///   true)
        /// </param>
        public RectangleLedGroup(ICueDevice device, RectangleF rectangle, float minOverlayPercentage = 0.5f, bool autoAttach = true) : base(device, autoAttach)
        {
            Rectangle = rectangle;
